...
## static v. shared library situation

At the moment, the build system naturally produces static libraries. (Well,
but, on MacOS these also somehow work when shared libraries are expected.
We live in an age of wonders.) Making shared libraries be a genteel option
(or having both static and shared be generated by default) is very much a
work in progress. But notes about a stopgap Linux measure, including truly
horrifying command-line library-makin' invocations, live temporarily in
the file `linux-shared-lib-build.txt`.
